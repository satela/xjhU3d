...
// 		Debug.Log(findPath);
	...
	public static GameObject[] GetPrefabList(string strDir, bool bIncludeMesh, bool bRecursively, int nMaxFile, out int nOutFindFile)
	{
		...
		if (bRecursively)
			 retArray = GetResourceFileRecursively<GameObject>(strDir, null, nMaxFile, false, out nOutFindFile);
		else retArray = GetResourceFiles<GameObject>(strDir, null, nMaxFile, false, out nOutFindFile);
...
// 					Debug.Log(((GameObject)retArray[nCount]).name);
	...
	public static ObjectNode[] GetPrefabList(string strDir, bool bIncludeMesh, bool bRecursively, int nMaxFile, bool bAsyncLoad, out int nOutFindFile)
	{
		ArrayList	retArray;
		int			nCount = 0;

		if (bRecursively)
			 retArray = GetResourceFileRecursively<GameObject>(strDir, null, nMaxFile, bAsyncLoad, out nOutFindFile);
		else retArray = GetResourceFiles<GameObject>(strDir, null, nMaxFile, bAsyncLoad, out nOutFindFile);
...
// 					Debug.Log(((GameObject)retArray[nCount]).name);
	...
	public static ObjectNode[] GetTextureList(string strDir, bool bRecursively, int nMaxFile, bool bAsyncLoad, out int nOutFindFile)
	{
		ArrayList retArray;
		if (bRecursively)
			 retArray = GetResourceFileRecursively<Texture>(strDir, null, nMaxFile, bAsyncLoad, out nOutFindFile);
		else retArray = GetResourceFiles<Texture>(strDir, null, nMaxFile, bAsyncLoad, out nOutFindFile);
		return ArrayListToObjectNodes(retArray);
	}
	...
	public static Material[] GetMaterialList(string strDir, bool bRecursively, int nMaxFile, out int nOutFindFile)
	{
		if (bRecursively)
		{
			ArrayList retArray = GetResourceFileRecursively<Material>(strDir, null, nMaxFile, false, out nOutFindFile);
			return NgConvert.ToArray<Material>(retArray);
		}
		return NgConvert.ToArray<Material>(GetResourceFiles<Material>(strDir, null, nMaxFile, false, out nOutFindFile));
	}
	...
	public static GameObject[] GetCurvePrefabList(string strDir, string containFileName, bool bRecursively, int nMaxFile, out int nOutFindFile)
	{
		if (bRecursively)
		{
			ArrayList retArray = GetResourceFileRecursively<GameObject>(strDir, containFileName, nMaxFile, false, out nOutFindFile);
			return NgConvert.ToArray<GameObject>(retArray);
		}
		return NgConvert.ToArray<GameObject>(GetResourceFiles<GameObject>(strDir, containFileName, nMaxFile, false, out nOutFindFile));
	}
	...
	public static ObjectNode[] GetTexturePathList(string strDir, bool bRecursively, int nMaxFile, out int nOutFindFile)
	{
		ArrayList retArray;
		if (bRecursively)
			 retArray = GetResourceFileRecursively<Texture>(strDir, null, nMaxFile, false, out nOutFindFile);
		else retArray = GetResourceFiles<Texture>(strDir, null, nMaxFile, false, out nOutFindFile);
		return ArrayListToObjectNodes(retArray);
	}
	...
	// ----------------------------------------------------------------------------------------------------------------------------------
	public static ArrayList GetResourceFileRecursively<TT>(string strDir, string containFileName, int nMaxFile, bool bAsyncLoad, out int nOutFindFile) where TT : Object
	{
#if UNITY_WEBPLAYER
		Debug.LogError("In WEB_PLAYER mode, you cannot run the FXMaker.");
		Debug.Break();
		...
		retArray = GetResourceFiles<TT>(strDir, containFileName, nMaxFile, bAsyncLoad, out nFindFile);
		...
		foreach (DirectoryInfo dirInfo in info)
		{
			retArray.AddRange(GetResourceFileRecursively<TT>(NgFile.CombinePath(strDir, dirInfo.Name), containFileName, nMaxFile, bAsyncLoad, out nFindFile));
			nOutFindFile += nFindFile;
			if (nMaxFile != 0)
			{
				nMaxFile = nMaxFile - nFindFile;
	...
	public static ArrayList GetResourceFiles<TT>(string strDir, string containFileName, int nMaxFile, bool bAsyncLoad, out int nOutFindFile) where TT : Object
	{
#if UNITY_WEBPLAYER
		Debug.LogError("In WEB_PLAYER mode, you cannot run the FXMaker.");
		Debug.Break();
